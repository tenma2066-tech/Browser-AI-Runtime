...
## 7.5 Phase 2a 後半（双方向ループ）― 記憶が予測を助ける

2a 前半は記憶に「書く」だけだった。後半では **記憶を予測に読み戻す**ことで、
「記憶が認知を助ける」ことを証明する。非パラメトリック記憶（kNN-LM 的）で実装。

- **連想の記銘**: Memory Fabric に `assoc`（連想値＝次文字のトークンID）を追加。
  「経験」時に `文脈ベクトル(encode) → 実際に来た次文字` を記憶に書く。
- **予測時のブレンド**: 現在の文脈を encode → 想起 → 想起先の `assoc` を集約して
  事前分布 `p_mem` を作り、Core のパラメトリック分布 `p_core` とゲート付きブレンド:
  `p = (1-λ)·p_core + λ·p_mem`、`λ = topSim > gate ? lamMax·topSim : 0`
  （既定 lamMax=0.9, gate=0.6）。強い一致があるときだけ記憶が効く。
- **設計上の注意**: 未知文字は UNK に潰れて連想が退化するため、実験文は
  「既知文字の新しい組み合わせ」（例:「猫は傘を持って公園で眠る」）を使う。

### 追加の受け入れ条件

5. **[双方向ループ]** 驚く文を一度「経験」して記憶に書くと、**重みを変えずに**
   再遭遇時の平均 surprise が下がる。経験前は memory ON でも下がらない（対照）。

Node 実測: memory OFF 3.51 → 経験後 memory ON 1.39（約60%低下）、core.step は
不変（勾配学習でなく記憶の効果）、fabric は assoc 追加後も bit 一致。
