...
| **Phase 5** | 生成能力（小型因果 LM 自前実装・GitHub 知識データ + RAG・WebGPU） | ⬜ 次 |
...
- **5c**: 知識は **GitHub データ + RAG**（増やせば増える）⊕ ローカル個人記憶で接地
  して生成 + チャット UI ＝「GPT 的だがユーザーごとに育つ」。

方針: 配信は **GitHub Pages** のまま、**モデルは小型**に保ち、**知識はモデルの
大きさでなく GitHub 上のデータで持つ**（RAG）。推論は完全クライアント・自作エンジン
（既存 AI API 非依存）。詳細は ADR-0003。
